...
<jstl:if test="${deleteBet == true}">
		<div class="alert alert-success">
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			<spring:message code="moderator.deleteBet.ok" />
		</div>
</jstl:if>

<jstl:if test="${deleteBet == false}">
		<div class="alert alert-danger">
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			<spring:message code="moderator.deleteBet.error" />
		</div>
</jstl:if>
...
<display:table name="bets" pagesize="10" class="table" requestURI="${requestURI}" id="row">
	...
	<display:column sortable="false">
		<a href="moderator/detailsRejected.do?betId=${row.id}" >
			${details}
		</a>
	</display:column>
	...
	<display:column sortable="false">
		<a href="moderator/accept1.do?betId=${row.id}" >
			${action}
		</a>
	</display:column>
	
	<spring:message code="moderator.delete" var="delete" />
	<display:column sortable="false">
		<a href="moderator/deleteBet.do?betId=${row.id}" >
			${delete}
		</a>
	</display:column>
